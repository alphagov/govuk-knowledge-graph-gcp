# Import Publisher data into BigQuery

The Publisher database is backed up daily to files in AWS S3, which are copied into a GCP bucket by the [`govuk-s3-mirror`][govuk-s3-mirror].
...
1. Fetch the Publisher database backup file.  It knows where to find it from environment variables that are set when the [workflow][workflow-terraform] starts the virtual machine ([more details][docker]).
...
Local testing is difficult, because the Publisher database is large.  It can be easier to do the following:
