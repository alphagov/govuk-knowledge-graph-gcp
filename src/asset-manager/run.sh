...
# TODO - unsure if this is the correct database (suppose it's whatever the mongorestore restores it to?)
...
# Create a dataset in mongodb of relevant assets.
mongosh ${DATABASE} ${QUERY}

# 1. Export that dataset
# 2. Upload it to a cloud bucket
# TODO - this is likely an unnecessary step?
mongoexport \
  --quiet \
