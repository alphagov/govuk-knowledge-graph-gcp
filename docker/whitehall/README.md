...
This VM runs scripts that fetch the backup file, extract selected tables as plain text, and uploads them into BigQuery tables. There is no need to restore the backup to an actual running database.
...
The rest of the code is in [`src/whitehall`][src].  When the VM starts, the `entrypoint.sh` script fetches that code and runs it.  This separation avoids having to rebuild the docker image when only changing the code that it runs.
